	...
	if test "x$want_boost" = "xyes"; then
		...
		if test "$ac_boost_path" != ""; then
			...
			BOOST_CPPFLAGS="-I$ac_boost_path/include"
                        ac_boost_dir_custom=$ac_boost_path
		else
			...
			ac_boost_dir_custom=$ac_boost_path_tmp
		fi
...
[
	...
	if test "x$want_boost" = "xyes"; then
		if test "$ac_boost_path" != ""; then
			AC_MSG_CHECKING([Set the root directory for boost library])
			AC_MSG_RESULT([yes])	
			ac_boost_dir_custom=$ac_boost_path
		else
			AX_BOOST([1.45],[],[echo "boost not found"
		   					exit 202])
		fi
	else
		AX_BOOST([1.45],[],[echo "boost not found"
