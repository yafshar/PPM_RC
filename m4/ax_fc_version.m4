...
AC_DEFUN([AX_FC_VERSION_OUTPUT],[
	...
	dnl Compile and link our simple test program by passing a flag (argument
	dnl 1 to this macro) to the Fortran 90 compiler in order to get "version" output 
	...
	[AC_CACHE_CHECK([how to get the version output from $FC],
		...
		[AC_LANG_ASSERT(Fortran) 
			AC_COMPILE_IFELSE([AC_LANG_PROGRAM()],
				[ac_cv_prog_fc_version= 
					dnl Try some options frequently used verbose output
					for ac_version in -V -version --version +version -qversion; do
						AX_FC_VERSION_OUTPUT($ac_version)
						dnl look for "copyright" constructs in the output
