...
private:
    ...
    template<unsigned int A, unsigned int N> struct CTpowseries {
        static const unsigned int recurse=(1+A)*CTpowseries<A*A, N/2>::value;
        static const unsigned int value=  (N&1) ? 1+A*recurse : recurse;
    };

    template<unsigned int A> struct CTpowseries<A, 0> {
        static const unsigned int value=0;
    };

    template<unsigned int A> struct CTpowseries<A, 1> {
        static const unsigned int value=1;
    };
    ...
    template <unsigned int A, unsigned int B, unsigned int m> struct CTmultmod {
        // (A/2)*(B*2) mod m
    ...
    template <unsigned int B, unsigned int m> struct CTmultmod<0, B, m> {
        static const unsigned int value=0;
    };
...
template <> inline void Saru::advanceWeyl<1>() {
...
template <unsigned int seed> inline Saru Saru::fork() const {
...
template <unsigned int steps> inline unsigned int Saru::u32() {
  advanceLCG<steps>();
  advanceWeyl<steps>();
...
template <unsigned int steps> inline float Saru::f() {
...
template <unsigned int steps> inline float Saru::f(float low, float high) {
...
template <unsigned int steps> inline double Saru::d() {
...
template <unsigned int steps> inline double Saru::d(double low, double high) {
    const double TWO_N32 = 0.232830643653869628906250e-9; /* 2^-32 */
    signed int v=(signed int)u32<steps>(); // deliberate cast to signed int for conversion speed
