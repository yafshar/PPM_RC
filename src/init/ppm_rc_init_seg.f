      ...
      SUBROUTINE DTYPE(ppm_rc_init_seg)(info)
        ...
        & ppm_param_decomp_bisection,ppm_rank,ppm_param_decomp_cartesian
...
!            ! ghost update
!            !-------------------------------------------------------------------------
!            CALL iomesh%map_ghost_get(info)
!            or_fail("iomesh%map_ghost_get")
!
!            CALL image%map_ghost_push(iomesh,info)
!            or_fail("image%map_ghost_push")
!
!            CALL iomesh%map_isend(info)
!            or_fail("mesh%map_send")
!
!            CALL image%map_ghost_pop(iomesh,info)
!            or_fail("image%map_ghost_pop")
!
!         END SELECT

!            !check if all went well on image
!            WRITE(outputname,"(A)")"start"
!            CALL DTYPE(ppm_rc_write_image)(image,iomesh, &
!            &    TRIM(outputname),info)
!            or_fail("ppm_rc_write_image")

! #if   __DIME == __2D
!            CALL ppm_rc_convolve(RESHAPE((/.11111111111_MK,.11111111111_MK,.11111111111_MK,.11111111111_MK, &
!            & .11111111111_MK,.11111111111_MK,.11111111111_MK,.11111111111_MK,.11111111111_MK/),(/3,3/)),image,iomesh,info)
!            or_fail("ppm_rc_convolve")

!            CALL DTYPE(ppm_rc_median)(1,image,iomesh,info)
!            or_fail_alloc("ppm_rc_median")


!            WRITE(outputname,"(A)")"Filter"
!            CALL DTYPE(ppm_rc_write_image)(image,iomesh, &
!            &    TRIM(outputname),info)
!            or_fail("ppm_rc_write_image")
! #endif
...
!         ALLOCATE(ppm_t_field::imaget,STAT=info)
!         or_fail_alloc('Failed to allocate field.')
!
!         CALL imaget%create(1,info,name="image_pixel_intensity")
!         or_fail("Create field failed!" )
!         ! Create a field for image density
!
!         CALL imaget%discretize_on(iomesh,info)
!         or_fail("imaget discretize_on failed!")
!         ! Descretize the field over the mesh


!         l=20
!         ALLOCATE(nsde(l),STAT=info)
!         nsde=zero
!
!         DO i=1,l
!            sbpitr => iomesh%subpatch%begin()
!            DO WHILE (ASSOCIATED(sbpitr))
!               NULLIFY(wp1_r,wp2_r)
!
!               CALL sbpitr%get_field(image,wp1_r,info)
!               or_fail("Failed to get field wp1_r data.")
!
!               CALL sbpitr%get_field(imaget,wp2_r,info)
!               or_fail("Failed to get field wp2_r data.")
!
!               wp2_r=wp1_r
!
!               sbpitr => iomesh%subpatch%next()
!            ENDDO
!
!
!            CALL DTYPE(ppm_rc_gc)(image,iomesh,info)
!            or_fail("ppm_rc_gc")
!
!            stdout("step=",i)
!
...
!
! #if   __DIME == __2D
!               nsde(i)=SUM( (wp1_r(3:nnodes(1)-2,3:nnodes(2)-2)-wp2_r(3:nnodes(1)-2,3:nnodes(2)-2))* &
!               &            (wp1_r(3:nnodes(1)-2,3:nnodes(2)-2)-wp2_r(3:nnodes(1)-2,3:nnodes(2)-2)))
!
!               !nsde(i)=nsde(i)/sum(wp1_r(2:nnodes(1)-1,2:nnodes(2)-1)*wp1_r(2:nnodes(1)-1,2:nnodes(2)-1))
! #elif __DIME == __3D
!               nsde(i)=SUM( (wp1_r(3:nnodes(1)-2,3:nnodes(2)-2,3:nnodes(3)-2)-wp2_r(3:nnodes(1)-2,3:nnodes(2)-2,3:nnodes(3)-2))* &
!               &            (wp1_r(3:nnodes(1)-2,3:nnodes(2)-2,3:nnodes(3)-2)-wp2_r(3:nnodes(1)-2,3:nnodes(2)-2,3:nnodes(3)-2)))
! #endif
!
!               wp2_r=wp1_r
!               sbpitr => iomesh%subpatch%next()
...
!          ENDDO !i=1,l
!
!          CALL DTYPE(ppm_rc_write_image)(image,iomesh,"gausscurv",info)
!          or_fail("ppm_rc_write_image")
!
!         stdout_f('(5(g20.10,1x))','nsde(1:5)')
!         stdout_f('(5(g20.10,1x))','nsde(6:10)')
!         stdout_f('(5(g20.10,1x))','nsde(11:15)')
!         stdout_f('(5(g20.10,1x))','nsde(16:20)')
!
!         CALL mpi_barrier(ppm_comm,info)
!         CALL mpi_finalize(info)
!         STOP
